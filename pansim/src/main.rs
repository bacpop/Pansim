...
fn main() -> io::Result<()> {
    ...
    .arg(Arg::new("core_mu")
        .long("core_mu")
        .help("Average core SNP mutation rate (per site per genome per generation in core genome). Must be > 0.0.")
        .required(false)
        .default_value("0.05"))
    .arg(Arg::new("HR_rate")
        .long("HR_rate")
    ...
    .arg(Arg::new("rate_genes1")
        .long("rate_genes1")
        .help("Average number of accessory genes that are gained/lost per site per genome per generation in gene compartment 1. Must be >= 0.0.")
        .required(false)
        .default_value("1.0"))
    .arg(Arg::new("rate_genes2")
        .long("rate_genes2")
        .help("Average number of accessory genes that are gained/lost per site per genome per generation in gene compartment 2. Must be >= 0.0.")
        .required(false)
    ...
    .arg(Arg::new("verbose")
        .long("verbose")
        .help("Prints per generation information.")
        .required(false)
        .takes_value(false))
    ...
    .arg(Arg::new("genome_size_penalty")
        ...
        .help("Multiplier for each gene difference between avg_gene_freq and observed value.")
        .required(false)
        .default_value("0.99"))
    .arg(Arg::new("competition_strength")
        .long("competition_strength")
        .help("Strength of competition felt by strain to all others. 0.0 = no competition")
        .required(false)
        .default_value("0.0"))
    .get_matches();
